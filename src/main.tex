...
\begin{itemize}
    ...
    A \times B$
    \item Proving anything related to relations essentially boils down to applying
...
\begin{itemize}
    ...
    \begin{proof}
        ...
        if $z \le 0$. Secondly, it is an injection, since $\forall z \ne z' \in
        \mathbb{Z}, f(z) \ne f(z')$.
    \end{proof}
    \item It was George Cantor who first recognized that $|\mathbb{N}| \ne
    |\mathbb{R}|$ by proving that there are no surjective function $f:
    ...
    \item We can prove that $\aleph_0 < \aleph_1$ with the same argument that was used
    to prove that $|\mathbb{Q}| = \aleph_0$
    ...
    \begin{proof}
        ...
        following cases. \\
        \underline{Case 1}: If $a \notin f(a)$ then $a \in B$,
        consequently $f(a) = B$ is impossible, since it would imply that $a \in
        B$. \\
    ...
    \item It has been proven that this hypothesis cannot be proven true or
    false. Therefor, accepting it as true leads to one version of set theory,
    while accepting it as true leads to another, both different, but valid
    versions
\end{itemize}
